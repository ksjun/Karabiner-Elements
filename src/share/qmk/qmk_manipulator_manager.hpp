...
#include "qmk/qmk_manipulator.hpp"

namespace krbn {
namespace grabber {
namespace device_grabber_details {
class qmk_manipulator_manager final {
public:
  qmk_manipulator_manager(void) {
  ...
  void update(const core_configuration::details::profile& profile) {
    ...
    //   }
    // }
    if (auto m = make_manipulator()) {
      manipulator_manager_->push_back_manipulator(m);
    }
  }

private:
  std::shared_ptr<manipulator::manipulators::base> make_manipulator() const {
    try {
      return std::make_shared<manipulator::manipulators::qmk_manipulator::qmk>();
    } catch (const std::exception& e) {
      logger::get_logger()->error(e.what());
    }
    return nullptr;
